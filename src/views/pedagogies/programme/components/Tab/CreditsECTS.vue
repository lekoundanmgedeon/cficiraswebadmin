  ...
  <div class="row">
    ...
    <div class="col-12 mb-4">
      ...
      <p class="text-muted small mb-0">
        ...
        Pilotez les portefeuilles de crédits des apprenants, attribuez les équivalences extérieures et validez la capitalisation.
      </p>
    </div>
        ...
        <div class="card-body p-3">
          <div class="row g-3 align-items-center">
            <div class="col-md-4">
              ...
              <select class="form-select border-0 shadow-sm" v-model="selectedClasse" @change="loadStudentsCredits">
                <option value="">Choisir une promotion...</option>
                <option v-for="c in mockClasses" :key="c" :value="c">{{ c }}</option>
              </select>
            </div>
            <div class="col-md-5">
              <label class="form-label small fw-semibold text-muted mb-1">Rechercher un apprenant</label>
              <div class="input-group input-group-sm bg-white rounded shadow-sm border-0">
                <span class="input-group-text bg-white border-0"><i class="bi bi-search text-muted"></i></span>
                <input type="text" class="form-control border-0 py-2" placeholder="Nom ou matricule..." v-model="searchQuery" :disabled="!selectedClasse" />
              </div>
            </div>
    ...
    <div class="col-12">
      <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
        <div class="card-header bg-white border-0 pt-4 px-4 pb-2 d-flex justify-content-between align-items-center">
          ...
          <button class="btn btn-outline-primary btn-xs border shadow-sm" :disabled="!selectedClasse" @click="openEquivalenceModal">
            ...
            <table class="table table-hover align-middle mb-0 text-center">
              ...
              <tbody>
                <tr v-for="student in filteredStudents" :key="student.matricule">
                  ...
                  <!-- S1 & S2 -->
                  <td><span class="fw-semibold">{{ student.ectsS1 }} / 30</span></td>
                  <td><span class="fw-semibold">{{ student.ectsS2 }} / 30</span></td>
                  
                  <!-- Équivalences (ex: transfert d'une autre école) -->
                  <td>
                    <span v-if="student.equivalences > 0" class="badge bg-soft-warning text-warning fw-bold">
                  ...
                  <td>
                    <div class="d-flex align-items-center justify-content-center">
                      <div class="progress w-100 me-2" style="height: 6px; background-color: #f0f2f5;">
                        <div 
                      ...
                      <span class="text-xs font-monospace text-muted">{{ calculateTotal(student) }}/120</span>
                    </div>
                  </td>

                  <!-- État d'avancement -->
                  <td class="text-end pe-4">
                    <span class="badge px-3 py-1 rounded-pill fw-bold" :class="getStatusClass(calculateTotal(student))">
                ...
                <tr v-if="filteredStudents.length === 0">
                  <td colspan="7" class="text-center py-5 text-muted">
                    <p class="small mb-0">Aucun historique de crédit disponible pour cette sélection.</p>
                  </td>
                </tr>
              </tbody>
            </table>
...
const filteredStudents = computed(() => {
  return studentsCreditsList.value.filter(s => 
...
const openEquivalenceModal = () => {
  alert("Ouverture du formulaire d'injection d'ECTS extérieurs (Validation des Acquis de l'Expérience ou Transfert d'Université).");
};
</script>
