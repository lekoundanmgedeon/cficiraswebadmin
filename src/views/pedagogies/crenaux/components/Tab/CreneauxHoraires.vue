  ...
  <div class="row">
    ...
    <div class="col-12 mb-4">
      ...
      <p class="text-muted small mb-0">
        ...
        Organisez l'emploi du temps. Importez vos plannings en masse ou ajustez les créneaux manuellement (cours, formateurs, salles).
      </p>
    </div>
      ...
      <div class="row g-3">
          ...
          <div class="card border-0 shadow-sm rounded-4 h-100 bg-white">
            ...
            <div class="card-body p-4 d-flex flex-column justify-content-between">
              <p class="text-muted small mb-3">
                Téléversez un fichier de planification au format <code>.xlsx</code> ou <code>.csv</code> pour injecter l'emploi du temps d'un coup.
              </p>
              ...
              >
                ...
                <span class="text-muted text-uppercase" style="font-size: 10px;">
                  {{ uploadedFile ? `${(uploadedFile.size / 1024).toFixed(1)} Ko` : "ou cliquez pour parcourir" }}
                </span>
              </div>
              ...
              <div class="d-flex gap-2">
                ...
                <button v-if="uploadedFile" class="btn btn-light border py-2" @click="uploadedFile = null">
                  <i class="bi bi-x"></i>
                </button>
              </div>
            </div>
        ...
        <div class="col-lg-7">
          <div class="card border-0 shadow-sm rounded-4 h-100 bg-white">
            <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
              <h5 class="fw-bold text-dark mb-0">
                <i class="bi bi-pencil-square text-primary me-2"></i>{{ isEditing ? 'Ajuster un Créneau' : 'Ajout Tardif ou Modification' }}
              </h5>
            </div>
              ...
              <form @submit.prevent="saveSchedule">
                <div class="row g-2">
                  ...
                  <div class="col-md-4">
                    ...
                    <select class="form-select bg-light border-0 shadow-sm" v-model="form.classe" required>
                  ...
                  <div class="col-md-8">
                    <label class="form-label small fw-semibold text-muted mb-1">Matière & Formateur</label>
                    <select class="form-select bg-light border-0 shadow-sm" v-model="form.matiereFormateur" required>
                  ...
                  <!-- Salle -->
                  <div class="col-md-4">
                    <label class="form-label small fw-semibold text-muted mb-1">Salle de cours</label>
                    <select class="form-select bg-light border-0 shadow-sm" v-model="form.salle" required>
                  ...
                  <div class="col-md-4">
                    ...
                    <input type="date" class="form-control bg-light border-0 shadow-sm" v-model="form.date" required />
                  </div>
                  <!-- Horaires (Début / Fin) -->
                  <div class="col-md-2">
                    <label class="form-label small fw-semibold text-muted mb-1">Début</label>
                    <input type="time" class="form-control bg-light border-0 shadow-sm" v-model="form.heureDebut" required />
                  </div>
                  <div class="col-md-2">
                    <label class="form-label small fw-semibold text-muted mb-1">Fin</label>
                    <input type="time" class="form-control bg-light border-0 shadow-sm" v-model="form.heureFin" required />
                  </div>
                </div>

                <!-- Actions du formulaire -->
                <div class="text-end mt-4">
                  <button v-if="isEditing" type="button" class="btn btn-light border-0 me-2" @click="resetForm">
                    Annuler
                  </button>
    ...
    <div class="col-12">
      <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white border-0 pt-4 px-4 pb-2 d-flex justify-content-between align-items-center">
          ...
          <div class="d-flex gap-2">
            <select class="form-select form-select-sm border shadow-sm px-3 py-1" v-model="tableFilterClasse">
              ...
              <tbody>
                <tr v-for="slot in filteredSchedules" :key="slot.id" class="transition-all">
                  ...
                  <td class="text-end pe-4">
                    <button class="btn btn-link text-primary p-0 me-3" title="Modifier la séance" @click="editSlot(slot)">
                      <i class="bi bi-pencil-fill fs-6"></i>
                    </button>
                    <button class="btn btn-link text-danger p-0 border-0" title="Supprimer le créneau" @click="deleteSlot(slot.id)">
                ...
                <tr v-if="filteredSchedules.length === 0">
                  <td colspan="6" class="text-center py-5">
                    ...
                    <p class="small text-muted mb-0">Utilisez l'import de fichier ou le formulaire d'ajout.</p>
                  </td>
                </tr>
              </tbody>
            </table>
...
const mockSchedules = ref([
  { id: 1001, date: '2026-05-19', heureDebut: '08:30', heureFin: '11:30', classe: 'Master 1 Info', matiere: 'Frameworks Modernes (Vue.js 3 & Node)', formateur: 'Traoré Moussa', salle: 'Salle 102 (Labo)' },
  { id: 1002, date: '2026-05-19', heureDebut: '13:30', heureFin: '16:30', classe: 'Master 2 Info', matiere: 'Deep Learning & Vision par ordinateur', formateur: 'Alami Sanaa', salle: 'Amphi A' }
]);
...
const processImport = () => {
  alert(`Fichier [${uploadedFile.value.name}] analysé avec succès ! Ajout simulé de 24 nouveaux créneaux horaires dans la base de données.`);
...
const resetForm = () => {
  ...
  form.value = { classe: '', matiereFormateur: '', salle: '', date: '', heureDebut: '', heureFin: '' };
};
