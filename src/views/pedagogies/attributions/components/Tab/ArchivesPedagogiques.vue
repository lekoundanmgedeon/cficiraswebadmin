  ...
  <div class="row">
    ...
    <div class="col-12 mb-4">
      ...
      <p class="text-muted small mb-0">
        ...
        Consultez l'historique immuable des modules, matières et coefficients des années académiques
        clôturées.
      </p>
    </div>
      ...
      <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white border-0 pt-4 px-4 pb-2">
          <h5 class="fw-bold text-dark mb-0">
            <i class="bi bi-folder-symlink-fill text-warning me-2"></i>Maquette Structurelle —
            {{ selectedAnnee }}
          </h5>
        </div>
                  ...
                  <td>
                    <ul class="list-unstyled mb-0 small text-secondary">
                      <li v-for="(mat, idx) in archive.matieres" :key="idx" class="mb-1">
                        <i class="bi bi-dot text-warning"></i> {{ mat.nom }}
                        <span class="text-muted">(Coef. {{ mat.coef }})</span>
                      </li>
                    </ul>
                  </td>
                ...
                <tr v-if="filteredArchives.length === 0">
                  <td colspan="5" class="text-center py-5">
                    ...
                    <p class="small text-muted mb-0">
                      Modifiez les critères ou l'année de recherche.
                    </p>
                  </td>
                </tr>
              </tbody>
...
const filteredArchives = computed(() => {
  return mockModuleArchives.value.filter((arc) => {
    ...
    const matchesModule =
      arc.nom.toLowerCase().includes(term) || arc.code.toLowerCase().includes(term);
...
const downloadSyllabus = (code) => {
  alert(
    `Extraction du Syllabus historique officiel pour l'unité d'enseignement : ${code} (${selectedAnnee.value})`
  );
};
</script>
