    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
      <div>
        ...
        <p class="text-muted small mb-0">
          Consultez l'historique complet des sessions passées, les classements définitifs et les procès-verbaux officiels.
        </p>
      </div>

      <!-- Barre d'outils globale -->
      <div class="d-flex gap-2">
        <button class="btn btn-sm btn-light border text-secondary" @click="refreshArchives" title="Actualiser la liste">
      ...
      <div class="card-body p-3">
        <div class="row g-3">
          ...
          <div class="col-12 col-sm-6 col-md-3">
            <select v-model="filters.year" class="form-select form-select-sm fw-medium text-secondary">
          ...
          <div class="col-12 col-sm-6 col-md-4 col-lg-4">
            <select v-model="filters.status" class="form-select form-select-sm fw-medium text-secondary">
      ...
      <table class="table table-hover align-middle mb-0 text-sm">
        ...
        <tbody>
          <tr v-for="item in filteredArchives" :key="item.id">
            ...
            <!-- Code -->
            <td>
              <span class="badge bg-light text-secondary border font-monospace">{{ item.code }}</span>
            </td>

            <!-- Intitulé -->
            <td>
              <div class="fw-semibold text-dark mb-0 line-clamp-1">{{ item.libelle }}</div>
              <small class="text-xs text-muted">Archivé le {{ formatDate(item.date_archivage) }}</small>
            </td>
            ...
            <!-- Statut Final -->
            <td>
              <span class="badge px-2.5 py-1 text-xs fw-semibold" :class="getStatusClass(item.statut_final)">
                <i class="bi me-1" :class="item.statut_final === 'Clôturé' ? 'bi-check-circle-fill' : 'bi-x-circle-fill'"></i>
                {{ item.statut_final }}
              </span>
            </td>
...
const loadArchives = async () => {
  try {
    ...
    archives.value = [
      { id: 1, session: 2025, code: 'CONC-INFO-2025', libelle: 'Concours d\'Entrée en Génie Informatique', total_candidats: 145, statut_final: 'Clôturé', date_archivage: '2025-09-15T14:30:00Z' },
      { id: 2, session: 2025, code: 'CONC-MGT-2025', libelle: 'Concours Licence Management des Organisations', total_candidats: 89, statut_final: 'Clôturé', date_archivage: '2025-10-02T10:15:00Z' },
      { id: 3, session: 2024, code: 'CONC-MED-2024', libelle: 'Concours Spécial d\'Accès aux Études Médicales', total_candidats: 320, statut_final: 'Clôturé', date_archivage: '2024-07-20T16:00:00Z' },
      { id: 4, session: 2024, code: 'CONC-X-2024', libelle: 'Concours d\'Admission Session Extraordinaire (Annulé)', total_candidats: 12, statut_final: 'Annulé', date_archivage: '2024-08-05T11:00:00Z' }
    ];
  } catch (err) {
