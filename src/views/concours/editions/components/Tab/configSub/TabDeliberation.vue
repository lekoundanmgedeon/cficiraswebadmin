<template>
  <div class="animate__animated animate__fadeIn">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        ...
        <p class="text-muted small mb-0">
          Analysez les performances globales, fixez la note d'admission et générez les résultats officiels.
        </p>
      </div>
      ...
      <span 
        ...
        :class="isPublished ? 'bg-success-subtle text-success border-success' : 'bg-warning-subtle text-warning border-warning'"
      >
    ...
    <div class="row g-3 mb-4">
      <div class="col-12 col-md-4">
        <div class="card border shadow-sm rounded-3 p-3 bg-white h-100">
          ...
          <div class="d-flex align-items-baseline gap-2">
            <h2 class="fw-extrabold text-dark mb-0 font-monospace">{{ stats.moyenneGenerale || '11.45' }}</h2>
      ...
      <div class="col-12 col-md-4">
        <div class="card border shadow-sm rounded-3 p-3 bg-white h-100">
          <div class="text-muted text-xs text-uppercase fw-semibold mb-1">Taux d'Admissibilité Actuel</div>
          <div class="d-flex align-items-baseline gap-2">
            <h2 class="fw-extrabold text-success mb-0 font-monospace">{{ stats.tauxReussite || '64.2' }}%</h2>
          </div>
          <small class="text-muted text-xs mt-2 d-block">
            Soit <strong>{{ stats.admisCount || 45 }}</strong> candidats sur {{ stats.totalCandidats || 70 }}
          </small>
        </div>
      </div>
      ...
      <div class="col-12 col-md-4">
        <div class="card border border-primary-subtle shadow-sm rounded-3 p-3 bg-primary-subtle bg-opacity-10 h-100">
          ...
          <small class="text-muted text-xs d-block">Modifiez ce seuil pour ajuster automatiquement le classement.</small>
        </div>
      </div>
    </div>
        ...
        <table class="table table-hover align-middle mb-0 text-sm">
          ...
          <tbody>
            <tr 
              ...
              :key="cand.id"
              :class="{ 'table-success-subtle': cand.moyenne >= seuilAdmission, 'table-danger-subtle': cand.moyenne < seuilAdmission }"
            >
              <!-- Rang -->
              <td class="ps-3 font-monospace fw-bold">
                <span v-if="cand.moyenne >= seuilAdmission" class="text-dark">#{{ index + 1 }}</span>
              ...
              <td class="text-end pe-3">
                <span v-if="cand.moyenne >= seuilAdmission" class="badge bg-success text-white text-xs px-2.5 py-1 fw-bold rounded-pill">
                  Admis
                </span>
                <span v-else class="badge bg-danger text-white text-xs px-2.5 py-1 fw-bold rounded-pill">
                  Ajourné
                </span>
              </td>
            </tr>
      ...
      <div class="row align-items-center g-3">
        <div class="col-12 col-md-8">
          <h6 class="fw-bold text-dark mb-1">
            <i class="bi bi-shield-exclamation text-warning me-1"></i> Clôture officielle de la délibération
          </h6>
          <p class="text-muted small mb-0">
            La publication fige définitivement les notes, génère les numéros de PV officiels, et rend les résultats visibles sur les portails d'affichage ou espaces candidats. Cette action est irréversible.
          </p>
        </div>
...
const loadDeliberationData = async () => {
  try {
    ...
    stats.value = { moyenneGenerale: '12.18', tauxReussite: '66.6', admisCount: 2, totalCandidats: 3 };
    
    simulationList.value = [
      { id: 101, num_table: 'NUM-5092', nom: 'TRAORE', prenom: 'Moussa', moyenne: 15.25 },
      { id: 102, num_table: 'NUM-1104', nom: 'SOW', prenom: 'Fatoumata', moyenne: 11.80 },
...
const publierResultatsOfficiels = async () => {
  if (confirm('Attention : En publiant ces résultats, vous scellez définitivement le Procès-Verbal de délibération. Confirmer ?')) {
    try {
...
const annulerPublication = () => {
  if (confirm('Restaurer l\'état de brouillon ? (Les résultats ne seront plus visibles publiquement)')) {
