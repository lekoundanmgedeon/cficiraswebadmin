  ...
  <div class="animate__animated animate__fadeIn">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        ...
        <p class="text-muted small mb-0">
          Gérez la liste des postulants inscrits. L'intégration s'effectue par importation de fichiers standardisés.
        </p>
      </div>
      
      <div class="d-flex gap-2">
        <button class="btn btn-sm btn-light border text-secondary" @click="downloadTemplate" title="Télécharger le modèle Excel de base">
    ...
    <div class="row g-4">
      <div class="col-12 col-lg-4">
        <div class="card border border-dashed rounded-4 p-4 text-center bg-light-subtle h-100 d-flex flex-column justify-content-center min-h-dropzone">
          <input 
            type="file" 
            ref="fileInputRef" 
          ...
          <div v-if="!selectedFile">
            ...
            <p class="text-muted small px-3">
              Glissez-déposez votre fichier ici, ou <a href="#" @click.prevent="triggerFileSelect" class="text-decoration-none fw-semibold">parcourez vos fichiers</a>.
            </p>
          ...
          <div v-else class="animate__animated animate__fadeIn">
            ...
            <p class="text-muted small font-monospace mb-3">{{ (selectedFile.size / 1024).toFixed(1) }} KB</p>
            
            <div class="d-grid gap-2 px-3">
              <button class="btn btn-success btn-sm d-flex align-items-center justify-content-center gap-2" @click="uploadFile" :disabled="isUploading">
              ...
              <button class="btn btn-link btn-sm text-danger text-decoration-none" @click="cancelSelection" :disabled="isUploading">
                Annuler
              </button>
            </div>
          </div>
        ...
        <div class="row g-3">
          <div class="col-12 col-sm-6">
            <div class="card border shadow-sm rounded-3 p-3 bg-white">
              <div class="text-muted text-xs text-uppercase fw-semibold mb-1">Candidats Validés</div>
              <div class="d-flex align-items-center gap-2">
                <h3 class="fw-bold text-dark mb-0 font-monospace">{{ candidats.length }}</h3>
                <span class="badge bg-success-subtle text-success text-xs rounded-pill">Inscrits</span>
              </div>
            </div>
          </div>
          <div class="col-12 col-sm-6">
            ...
            <tbody>
              <tr v-for="(candidat, index) in candidats" :key="index">
                ...
                <td>
                  ...
                  <small class="text-muted text-xs font-monospace">Sexe : {{ candidat.sexe || 'M' }}</small>
                </td>
                <td class="text-muted font-monospace text-xs">{{ candidat.email || '—' }}</td>
                <td class="font-monospace text-xs">{{ candidat.telephone || '—' }}</td>
                <td class="text-end pe-3">
                  <button class="btn btn-sm btn-link text-danger p-1" @click="deleteCandidat(candidat.id, index)" title="Retirer ce candidat">
              ...
              <tr v-if="candidats.length === 0">
                <td colspan="5" class="text-center text-muted py-5">
                  ...
                  Aucun candidat sur la liste pour le moment. Intégrez votre fichier CSV ou Excel pour commencer.
                </td>
              </tr>
            </tbody>
          </table>
...
const uploadFile = async () => {
  ...
  try {
    ...
    notifyError(extractErrorMessage(err, 'Le format du fichier ou la structure des colonnes est incorrect.'));
  } finally {
