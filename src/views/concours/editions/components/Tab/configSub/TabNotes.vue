    ...
    >
      <div>
        ...
        <p class="text-muted small mb-0">
          ...
          enregistrées à la volée ou globalement.
        </p>
      </div>
    ...
    <div v-else class="animate__animated animate__fadeIn animate__fast">
      ...
      >
        ...
        <div class="d-flex gap-2">
          <button
            class="btn btn-xs btn-outline-secondary bg-white"
            @click="loadNotes"
          ...
          <tbody>
            <tr v-for="(row, index) in notesRows" :key="row.candidat_id">
              ...
              <td class="text-end pe-3">
                ...
                <span
                  v-else-if="row.note !== null && row.note !== undefined && !row.isModified"
...
const saveAllNotes = async () => {
  ...
  if (hasErrors) {
    return notifyError(
      "Veuillez corriger les notes supérieures à 20 ou négatives avant d'enregistrer."
    );
  }
