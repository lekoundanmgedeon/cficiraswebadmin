  ...
  <div class="simulator-container">
    ...
    <div class="col-12 mb-4">
      ...
      <p class="text-muted small mb-0">
        ...
        Ajustez les leviers stratégiques pour simuler l'impact financier immédiat sur l'année académique courante.
      </p>
    </div>
    ...
    <div class="row g-3">
        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white p-4 h-100">
          ...
          <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-1">
              ...
              <span class="badge bg-soft-primary text-primary font-monospace fw-bold">{{ scolariteModifier > 0 ? '+' : '' }}{{ scolariteModifier }}%</span>
            </div>
            <input type="range" v-model.number="scolariteModifier" min="-20" max="30" step="5" class="form-range custom-range" />
          ...
          <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-1">
              ...
              <span class="badge bg-soft-success text-success font-monospace fw-bold">{{ effectifModifier > 0 ? '+' : '' }}{{ effectifModifier }} %</span>
            </div>
            <input type="range" v-model.number="effectifModifier" min="-30" max="50" step="5" class="form-range custom-range" />
            <div class="d-flex justify-content-between text-xs text-muted">
          ...
          <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-1">
              ...
              <span class="badge bg-soft-danger text-danger font-monospace fw-bold">{{ vacatModifier > 0 ? '+' : '' }}{{ vacatModifier }}%</span>
            </div>
            <input type="range" v-model.number="vacatModifier" min="-10" max="25" step="5" class="form-range custom-range" />
            <div class="d-flex justify-content-between text-xs text-muted">
          ...
          <div class="pt-3 border-top border-light mt-auto">
            <button @click="resetSimulateur" class="btn btn-sm btn-light border text-secondary w-100 py-2">
              <i class="bi bi-arrow-counterclockwise me-1"></i> Réinitialiser les paramètres par défaut
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-7">
        <div class="card border-0 shadow-sm rounded-4 bg-white p-4 h-100">
          ...
          <div class="table-responsive mb-4">
            <table class="table align-middle text-center mb-0">
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-start ps-3 small fw-semibold text-dark">Chiffre d'Affaires Global</td>
                  <td class="font-monospace text-muted text-xs">{{ formatPrice(baseFinances.caActuel) }}</td>
                  <td class="font-monospace fw-bold text-primary">{{ formatPrice(projections.caProjete) }}</td>
                  <td class="text-end pe-3 font-monospace small fw-bold" :class="projections.caEcart >= 0 ? 'text-success' : 'text-danger'">
                ...
                <tr>
                  ...
                  <td class="font-monospace text-muted text-xs">{{ formatPrice(baseFinances.chargesActuelles) }}</td>
                  <td class="font-monospace fw-bold text-danger">{{ formatPrice(projections.chargesProjetees) }}</td>
                  <td class="text-end pe-3 font-monospace small fw-bold" :class="projections.chargesEcart <= 0 ? 'text-success' : 'text-danger'">
                    {{ projections.chargesEcart > 0 ? '+' : '' }}{{ formatPrice(projections.chargesEcart) }}
                  </td>
                </tr>
                ...
                <tr class="table-group-divider bg-light-subtle">
                  ...
                  <td class="font-monospace text-muted small fw-bold">{{ formatPrice(baseFinances.caActuel - baseFinances.chargesActuelles) }}</td>
                  <td class="font-monospace fw-bold text-success">{{ formatPrice(projections.resultatProjete) }}</td>
                  <td class="text-end pe-3 font-monospace fw-bold" :class="projections.resultatEcart >= 0 ? 'text-success' : 'text-danger'">
                    {{ projections.resultatEcart >= 0 ? '+' : '' }}{{ formatPrice(projections.resultatEcart) }}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="p-3 bg-light rounded text-xs text-muted border-start border-3 border-info mt-auto">
            <span class="fw-bold text-dark d-block mb-1"><i class="bi bi-briefcase me-1"></i>Note Prudentielle :</span>
            Cette projection part de l'hypothèse que l'élasticité du recrutement reste stable malgré les ajustements tarifaires. L'impact réel peut varier en fonction des taux effectifs d'encaissement de l'échéancier.
          </div>
        </div>
      </div>
    </div>
...
const projections = computed(() => {
  ...
  const chargesProjetees = Math.round(baseFinances.chargesActuelles * coefficientVacation * coefficientVolumeHeure);
