  ...
  <div class="echeancier-container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        ...
        <p class="text-muted small mb-0">
          ...
          Configuration des traites, fractionnement des scolarités et suivi des échéances par classe.
        </p>
      </div>
    ...
    <div class="row g-3 mb-4">
      <div v-for="plan in plansTypes" :key="plan.id" class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 bg-white p-3 border-top border-3" :class="'border-' + plan.color">
          <div class="d-flex justify-content-between align-items-start mb-2">
            <h6 class="fw-bold text-dark mb-0">{{ plan.nom }}</h6>
            <span class="badge bg-light text-dark border font-monospace text-xs">{{ plan.nombreTraites }} échéances</span>
          </div>
          <p class="text-muted text-xs mb-3">{{ plan.description }}</p>
          
          <!-- Traites détaillées -->
          <div class="timeline-traites small mb-3">
            <div v-for="(traite, idx) in plan.repartition" :key="idx" class="d-flex justify-content-between text-xs py-1 border-bottom border-light font-monospace">
              <span class="text-secondary"><i class="bi bi-arrow-right-short"></i> Traite {{ idx + 1 }} :</span>
              <span class="fw-bold text-dark">{{ traite.pourcentage }}% ({{ traite.periode }})</span>
            </div>
          </div>

          <div class="d-flex justify-content-between align-items-center pt-2">
            <span class="text-xs text-muted">Classes : <strong class="text-dark">{{ plan.classesAssociees }}</strong></span>
            <button class="btn btn-xs btn-light text-primary border-0 font-semibold py-1 px-2" style="font-size: 11px;">
    ...
    <div class="card shadow-sm border-0 rounded-4 overflow-hidden bg-white">
      <div class="card-header bg-white border-0 pt-4 px-4 pb-2 d-flex justify-content-between align-items-center">
        <h5 class="fw-bold text-dark mb-0">
          <i class="bi bi-person-lines-fill text-secondary me-2"></i>État des Échéances Individuelles (Mois en cours)
        </h5>
        <div class="d-flex gap-2">
          <select class="form-select form-select-sm text-xs shadow-none border" style="width: 150px;">
            ...
            <tbody>
              <tr v-for="traite in suiviTraites" :key="traite.id">
                ...
                <td class="ps-4 text-start">
                  ...
                  <small class="text-muted font-monospace text-xs">{{ traite.matricule }} • {{ traite.plan }}</small>
                </td>
                <!-- Numéro de la traite -->
                <td>
                  <span class="badge bg-light text-secondary border font-monospace">T{{ traite.numero }} / {{ traite.totalTraites }}</span>
                </td>
                <!-- Montant -->
                <td class="font-monospace fw-bold">{{ formatPrice(traite.montant) }}</td>
                <!-- Échéance -->
                <td class="font-monospace text-muted small">{{ formatDate(traite.dateEcheance) }}</td>
                <!-- Statut -->
                <td>
                  <span class="badge px-2 py-1 rounded-pill fw-bold" :class="getStatutClass(traite.statut)">
                    <i class="bi me-1" :class="getStatutIcon(traite.statut)"></i> {{ traite.statut }}
                  </span>
                </td>
                <!-- Retard -->
                ...
                <td class="text-end pe-4">
                  <div class="btn-group btn-group-xs">
                    <button @click="encaisserTraite(traite)" class="btn btn-sm btn-success border-0 py-1 px-2" :disabled="traite.statut === 'Payée'" title="Encaisser la traite">
                      <i class="bi bi-cash-coin"></i>
                    </button>
                    <button @click="envoyerRappel(traite)" class="btn btn-sm btn-light border py-1 px-2 text-danger" :disabled="traite.statut === 'Payée'" title="Relancer par SMS/Email">
...
const suiviTraites = ref([
  { id: 101, matricule: 'ETU-2026-001', etudiant: 'Moussa Diallo', plan: 'Plan Standard', numero: 2, totalTraites: 3, montant: 150000, dateEcheance: '2026-05-15', statut: 'En retard', joursRetard: 4 },
  { id: 102, matricule: 'ETU-2026-002', etudiant: 'Awa Ndiaye', plan: 'Plan Standard', numero: 2, totalTraites: 3, montant: 150000, dateEcheance: '2026-05-15', statut: 'Payée', joursRetard: 0 },
  { id: 103, matricule: 'ETU-2026-114', etudiant: 'Ibrahima Diallo', plan: 'Plan Executive', numero: 2, totalTraites: 2, montant: 300000, dateEcheance: '2026-04-30', statut: 'Contentieux', joursRetard: 19 },
  { id: 104, matricule: 'ETU-2026-204', etudiant: 'Oumar Sy', plan: 'Plan Mensuel', numero: 4, totalTraites: 6, montant: 75000, dateEcheance: '2026-05-25', statut: 'En attente', joursRetard: 0 }
]);
...
const formatDate = (dateStr) => {
  return new Date(dateStr).toLocaleDateString('fr-FR', { year: 'numeric', month: '2-digit', day: '2-digit' });
};
...
const encaisserTraite = (traite) => {
  ...
  alert(`Traite enregistrée avec succès pour ${traite.etudiant}. Le grand livre comptable a été mis à jour.`);
};

const envoyerRappel = (traite) => {
  alert(`Relance automatique envoyée à ${traite.etudiant} (${traite.matricule}).\nCanal utilisé : SMS & Notification Portail.`);
};
</script>
