  ...
  <div class="assistant-ia-container container-fluid py-2">
    <div class="col-12 mb-4">
      ...
      <p class="text-muted small mb-0">
        ...
        Pilotez l'ensemble de votre établissement : suivez le statut des inscriptions, auditez la saisie des notes, simulez les délibérations de fin de cycle et suivez les proclamations de résultats.
      </p>
    </div>

    <div class="row g-3">
      <div class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 bg-white p-3 h-100 overflow-auto" style="max-height: 520px;">
          ...
          <div class="mb-3">
            ...
            <div class="d-grid gap-1">
              <button
                @click="askShortcut('Fais un rapport statistique rapide sur les inscriptions et la parité de l\'année.')"
          ...
          <div class="mb-3">
            ...
            <div class="d-grid gap-1">
              <button
                @click="askShortcut('Quelles sont les matières qui bloquent actuellement la complétude des notes de fin de semestre ?')"
                class="btn btn-light btn-sm text-start p-2 rounded text-secondary border-0 btn-prompt"
              >
          ...
          <div class="mb-3">
            <h6 class="fw-bold text-dark mb-2 small text-uppercase text-secondary tracking-wider">
              <i class="bi bi-shield-check text-primary me-2"></i>Gestion des Résultats & Délibérations
            </h6>
            <div class="d-grid gap-1">
              <button
                @click="askShortcut('Donne-moi le taux de réussite prédictif par classe avant délibération.')"
                class="btn btn-light btn-sm text-start p-2 rounded text-secondary border-0 btn-prompt"
              >
                <i class="bi bi-graph-up me-2 text-primary"></i> Estimations de Réussite
              </button>
              <button
                @click="askShortcut('Quels sont les étudiants éligibles au rachat ou secours du jury pour cette session ?')"
                class="btn btn-light btn-sm text-start p-2 rounded text-secondary border-0 btn-prompt"
              >
          ...
          <div class="mb-3">
            ...
            <div class="d-grid gap-1">
              <button
                @click="askShortcut('Quel est l\'état d\'avancement de la publication des procès-verbaux globaux et des bulletins ?')"
                class="btn btn-light btn-sm text-start p-2 rounded text-secondary border-0 btn-prompt"
              >
          ...
          <div class="mt-auto p-3 bg-soft-primary rounded text-xs text-muted">
            ...
            L'assistant synchronise en temps réel les bases relationnelles 2026 : de la scolarité aux procès-verbaux de proclamation.
          </div>
        </div>
      </div>

      <div class="col-md-8">
        <div class="card border-0 shadow-sm rounded-4 bg-white d-flex flex-column" style="height: 520px">
...
const sendMessage = () => {
  ...
  setTimeout(() => {
    ...
    if (searchStr.includes('parité') || searchStr.includes('statistique') || searchStr.includes('effectif') || searchStr.includes('inscription')) {
      aiResponse =
    ...
    else if (searchStr.includes('matière') || searchStr.includes('retard') || searchStr.includes('bloquent') || searchStr.includes('notes')) {
      aiResponse =
    ...
    else if (searchStr.includes('taux') || searchStr.includes('réussite') || searchStr.includes('prédictif')) {
      aiResponse =
    ...
    } else if (searchStr.includes('rachat') || searchStr.includes('secours') || searchStr.includes('éligibles')) {
      aiResponse =
    ...
    else if (searchStr.includes('proclamation') || searchStr.includes('publication') || searchStr.includes('bulletin') || searchStr.includes('pv')) {
      aiResponse =
