 ...
 * cliquer ne faisait rien. Trois de ses `<li>` partageaient le même
 * `id="sales-tab"`. Elle affectait `etudiant.value = response` sans déballer
 * l'enveloppe `{ success, data }` du backend, et servait la photo depuis un
 * `http://localhost:3500` codé en dur.
 *
 * Elle appelait surtout `GET /etudiants/:id`, **un endpoint qui n'existe pas**
 * (404). L'étudiant est retrouvé dans l'annuaire, projeté depuis les inscriptions.
 *
 * Son second onglet (« Fiche académique ») présentait des champs d'agent — grade,
 * fonction, unité de service, département — étrangers au modèle étudiant. Il cède
 * la place au parcours académique, qui, lui, correspond à un endpoint réel.
 *
 * Aucun bouton « Modifier » : `PUT /etudiants/:id` n'existe pas davantage.
 */

const router = useRouter();
const etudiantStore = useEtudiantStore();
const { item: etudiant, listLoading } = storeToRefs(etudiantStore);
...
<template>
  <div>
    ...
    <LoadingSpinner v-if="listLoading && !etudiant" />

    <EmptyState
      v-else-if="notFound || !etudiant"
      title="Étudiant introuvable"
      description="Cette fiche n'existe pas, ou l'étudiant n'a encore aucune inscription."
    />
