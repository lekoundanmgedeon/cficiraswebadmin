<template>
  <div class="py-4 px-2">
    ...
    <div id="cyclesAccordion" class="accordion custom-robust-accordion">
      <div
        v-for="(cycle, cIndex) in cycles"
        :key="cycle.name"
        class="accordion-item mb-3 border shadow-sm"
      >
        <h2 :id="'heading' + cIndex" class="accordion-header">
          <button
        ...
        >
          <div class="accordion-body bg-light-subtle p-4">
            ...
            <div class="row g-3">
              <div v-for="filiere in cycle.filieres" :key="filiere.name" class="col-12">
                <div
                ...
                >
                  ...
                  <!-- Grille des Niveaux -->
                  <!-- `v-if` et `v-for` cohabitaient sur le même élément : le v-if
                       portait sur la collection entière mais était réévalué à
                       chaque itération, et son `v-else` ne pouvait donc jamais
                       s'afficher. La condition remonte d'un cran. -->
                  <div class="row g-2 ms-4">
                    <template v-if="filiere.niveaux?.length">
                      <div
                    ...
                    <div v-else class="col-12 text-muted small ps-2">
...
/** Arborescence cycles → filières, servie par /cycles/tree/filieres. */
const cycles = computed(() => cycleStore.architecture);

onMounted(() => cycleStore.fetchArchitecture());
</script>
