          ...
          <table class="table table-bordered timetable-grid mb-0 text-center align-middle">
            ...
            <tbody>
              <tr v-for="slotTime in slotsHoraires" :key="slotTime.start">
                <!-- Colonne de l'heure -->
                <td
                  class="bg-light fw-semibold text-secondary small py-4 border-start-0 border-end-0"
                >
                  {{ slotTime.start }}<br /><span
                ...
                <td v-for="jour in joursSemaine" :key="jour.key" class="timetable-cell p-2">
                  ...
                  >
                    ...
                    <div class="small text-muted text-truncate mb-1" style="font-size: 11px">
                      ...
                      {{
                        viewType === 'classe'
                          ? getCourseAt(jour.dateStr, slotTime.start).formateur
                          : getCourseAt(jour.dateStr, slotTime.start).classe
                      }}
                    </div>

                    <span
                      class="badge bg-white text-dark border-0 shadow-xs px-2 py-1 fw-bold text-uppercase"
                      style="font-size: 9px"
                    >
                      <i class="bi bi-geo-alt-fill text-danger me-1"></i
                      >{{ getCourseAt(jour.dateStr, slotTime.start).salle }}
                    </span>
                  </div>
                </td>
              </tr>
